/*
	Latam Community - Comunidad Latino América.
...
public registrarDatos(playerid)
{
    ...
    format(str, sizeof(str), "%d,", yearActual);     					strcat(Consulta, str); //ao
...
stock mostrarMenuConfiguracionArma(playerid){
	...
	format(string, sizeof(string), "\n{7C7C7C}Armas rpidas");		strcat(Dialogo, string);
...
CMD:anuncio(playerid, params[])
{
    ...
    if(strfind(params, "~k~", true) != -1) 
        return SendClientMessage(playerid, -1, "> Intenta no usar muchos caracteres extraos.");
...
{
	if(isnull(params))
	   	return SendClientMessage(playerid, COLOR_AMARILLO, "> Escribiste mal el comando; {FFFFFF}/advertir [id] [razn]");
...
CMD:ir(playerid, params[]){
    ...
    new str[128];
    format(str, sizeof(str), "> Fuiste a la posicin de {%06x}%s{FFFFBB}.", colorJugador(i), Jugador[i][Nombre]);
    SendClientMessage(playerid, COLOR_AMARILLO, str);
